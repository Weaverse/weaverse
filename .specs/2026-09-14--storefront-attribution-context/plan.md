...
3. The module-private `bodyFreeCacheRequests` WeakSet marks the exact options
   ...
   `theme-settings` with varying bodies — the marked body slot holds the private
   `BODY_FREE_CACHE_SENTINEL` string rather than `undefined` (no-body external
   calls cannot collide), and an adjacent WeakSet-membership slot in the key is
   the unforgeable discriminator — a caller sending the sentinel bytes as its
   body still cannot reach the marked identity. No public API was added or changed, so
